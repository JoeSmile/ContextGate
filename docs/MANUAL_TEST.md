...
| 8.3 | llm-keys 加密入库 | `POST /api/admin/llm-keys` {key_alias, api_key_plaintext, provider?, base_url?} | 落库为密文(LLM_KEY_MASTER_KEY 已设时),列表不显示明文 |
...
| GAP-08 | LangFuse 观测 | harness 调不存在的 SDK 方法导致 usage/cost 未落库;`/chat/streaming` 无根 trace 产生孤儿 span | Important | ✅ 已修(2026-08-02:改 `update_current_observation` + streaming 根 observe;结构/时序本就正常,API 字段为 camelCase,详见 examples/qa/LANGFUSE.md §6) |
| | | | | |
